...
wtf.test(`State should ...`, (assert) => {
    let state1 = (0, newstate_1.stateify)({ string: "string", additional: true });
    // @ts-expect-error
    let state2 = (0, newstate_1.stateify)({ string: "string" });
});
wtf.test(`State should support exotic unions.`, (assert) => {
    let state = (0, newstate_1.stateify)({});
    state.update("string");
...
wtf.test(`Computed should compute a new state from two string states.`, (assert) => {
    let one = (0, newstate_1.stateify)("one");
    let two = (0, newstate_1.stateify)("two");
    let computed_state = (0, newstate_1.computed)([one, two], (one, two) => {
        return `${one} ${two}`;
    });
...
wtf.test(`Attributes<A> should support complex values.`, (assert) => {
    ...
    let array = (0, newstate_1.stateify)(attributes.array);
    ...
    let tuple = (0, newstate_1.stateify)(attributes.tuple);
    //tuple.update(["b", 1]);
...
wtf.test(`It should support assignment from empty string literal to any string.`, (assert) => {
    let string = (0, newstate_1.make_state)("");
});
wtf.test(`It should output undefined member values in object values.`, (assert) => {
    let state = (0, newstate_1.make_state)({ required: undefined });
...
wtf.test(`It should support updating optional object members to undefined values.`, (assert) => {
    let state = (0, newstate_1.make_state)({ object: { primitive: "a" } });
...
wtf.test(`It should support updating optional array members to undefined values.`, (assert) => {
    let state = (0, newstate_1.make_state)({ array: ["a"] });
...
wtf.test(`It should initialize optional members lazily when updated.`, (assert) => {
    let state = (0, newstate_1.make_state)({});
...
wtf.test(`It should filter arrays.`, (assert) => {
    let original = (0, newstate_1.make_state)(["a", "B", "c", "D", "e"]);
...
wtf.test(`A filtered array should be updated when elements are inserted at the start of the original array.`, (assert) => {
    let original = (0, newstate_1.make_state)([]);
    let filtered = original.filter((state) => state.compute((value) => value === value.toLowerCase()));
...
wtf.test(`Valueify should handle array states with values.`, (assert) => {
    let state = (0, newstate_1.valueify)((0, newstate_1.make_state)(["a", "b"]));
    assert.equals(state, ["a", "b"]);
});
wtf.test(`State arrays should be created with index signatures.`, (assert) => {
    let state = (0, newstate_1.make_state)(["a", "b", "c"]);
...
wtf.test(`State objects should be created with index signatures.`, (assert) => {
    let state = (0, newstate_1.make_state)({ string: "a", number: 0, boolean: false });
...
wtf.test(`ArrayState should support observers being added in mapped arrays.`, async (assert) => {
    let states = (0, newstate_1.make_state)([]);
...
wtf.test(`Attributes should be user-friendly.`, (assert) => {
    ...
    let state = (0, newstate_1.make_state)(value);
    ...
    assert.equals((0, newstate_1.valueify)(required_required), "reqreq");
    let required_optional = required.optional;
    assert.equals((0, newstate_1.valueify)(required_optional), "reqopt");
    let optional = (0, newstate_1.fallback)(state.optional, { required: "optreq2" });
    let optional_required = optional.required;
    assert.equals((0, newstate_1.valueify)(optional_required), "optreq");
    let optional_optional = optional.optional;
    assert.equals((0, newstate_1.valueify)(optional_optional), "optopt");
});
wtf.test(`Attributes should be composable from nested values and states.`, (assert) => {
    let attributes = {
        one: {
            one: (0, newstate_1.make_state)("a"),
            two: "b"
        },
        two: (0, newstate_1.make_state)({
            one: "c",
            two: "d"
        }),
        three: [
            {
                one: (0, newstate_1.make_state)("e"),
                two: "f"
            }
        ],
        four: (0, newstate_1.make_state)([
            {
                one: "g",
                two: "h"
            }
    ...
    assert.equals((0, newstate_1.valueify)(one_one), "a");
    let one_two = attributes.one.two;
    assert.equals((0, newstate_1.valueify)(one_two), "b");
    let two_one = attributes.two.one;
    assert.equals((0, newstate_1.valueify)(two_one), "c");
    let two_two = attributes.two.two;
    assert.equals((0, newstate_1.valueify)(two_two), "d");
    assert.equals((0, newstate_1.valueify)(attributes.three), [{ one: "e", two: "f" }]);
    assert.equals((0, newstate_1.valueify)(attributes.four), [{ one: "g", two: "h" }]);
});
wtf.test(`Array states should have rest functionality.`, (assert) => {
    let state = (0, newstate_1.make_state)(["a", "b"]);
    let spread = [...state];
...
wtf.test(`Object states should have spread functionality.`, (assert) => {
    let state = (0, newstate_1.make_state)({ one: "a", two: "b" });
    let spread = { ...state };
...
wtf.test(`Dynamic array elements should update properly when accessing deferred object members.`, (assert) => {
    let objects = (0, newstate_1.stateify)([
        {
            array: [
                {
                    array: []
    ...
    objects.element((0, newstate_1.stateify)(0)).deferred;
    objects.element((0, newstate_1.stateify)(0)).array[0].deferred;
});
wtf.test(`Dynamic ArrayState elements should support being updated.`, (assert) => {
    let array = (0, newstate_1.make_state)(["one", "two"]);
    let index = (0, newstate_1.stateify)(0);
...
wtf.test(`Dynamic ArrayState elements should throw an error when the index is invalid.`, async (assert) => {
    let array = (0, newstate_1.make_state)(["one", "two"]);
    let index = (0, newstate_1.stateify)(2);
...
wtf.test(`Lazily initialized ObjectStates should supporting being cleared.`, (assert) => {
    let object = (0, newstate_1.make_state)({});
...
wtf.test(`State<[string, string] | undefined> should be assignable to StateOrValue<[string, string] | undefined>.`, (assert) => {
    let attribute = (0, newstate_1.stateify)(["one", "two"]);
});
/*
...
wtf.test(`Squashed record arrays should support records being inserted before other records with identical keys.`, (assert) => {
    let records = (0, newstate_1.stateify)([
        { one: "a" }
    ]);
    let squashed = (0, newstate_1.squash)(records);
...
wtf.test(`Fallback states should be constructible from other fallback states.`, (asserts) => {
    let state = (0, newstate_1.make_state)({});
    let object = (0, newstate_1.fallback)(state.object, {});
    let string = (0, newstate_1.fallback)(object.string, "string");
...
wtf.test(`Fallback states should be initialized properly (undefined primitive).`, (assert) => {
    let underlying = (0, newstate_1.make_state)(undefined);
    let fallbacked = (0, newstate_1.fallback)(underlying, "default");
...
wtf.test(`Fallback states should be initialized properly (defined primitive different from the default value).`, (assert) => {
    let underlying = (0, newstate_1.make_state)("underlying");
    let fallbacked = (0, newstate_1.fallback)(underlying, "default");
...
wtf.test(`Fallback states should be initialized properly (defined primitive identical to the default value).`, (assert) => {
    let underlying = (0, newstate_1.make_state)("default");
    let fallbacked = (0, newstate_1.fallback)(underlying, "default");
    assert.equals(underlying.value(), "default");
    assert.equals(fallbacked.value(), "default");
});
...
wtf.test(`Fallback states should be initialized properly (undefined record).`, (assert) => {
    let underlying = (0, newstate_1.make_state)(undefined);
    let fallbacked = (0, newstate_1.fallback)(underlying, { value: "default" });
    assert.equals(underlying.value(), undefined);
...
wtf.test(`Fallback states should be initialized properly (defined record different from the default value).`, (assert) => {
    let underlying = (0, newstate_1.make_state)({ value: "underlying" });
    let fallbacked = (0, newstate_1.fallback)(underlying, { value: "default" });
...
wtf.test(`Fallback states should be initialized properly (defined record identical to the default value).`, (assert) => {
    let underlying = (0, newstate_1.make_state)({ value: "default" });
    let fallbacked = (0, newstate_1.fallback)(underlying, { value: "default" });
    assert.equals(underlying.value(), { value: "default" });
    assert.equals(fallbacked.value(), { value: "default" });
});
...
wtf.test(`Fallback states should synchronize underlying record states using insert, remove and update events.`, (assert) => {
    let underlying = (0, newstate_1.make_state)({});
    let fallbacked = (0, newstate_1.fallback)(underlying, {});
    let events = [];
...
wtf.test(`Merge should merge { a: "one" } with { a: "one" }.`, (assert) => {
    let one = (0, newstate_1.stateify)({ a: "one" });
    let two = (0, newstate_1.stateify)({ a: "one" });
    let merged = (0, newstate_1.merge)(one, two);
    assert.equals((0, newstate_1.valueify)(merged), { a: "one" });
});
wtf.test(`Merge should merge { a: "one" } with {}.`, (assert) => {
    let one = (0, newstate_1.stateify)({ a: "one" });
    let two = (0, newstate_1.stateify)({});
    let merged = (0, newstate_1.merge)(one, two);
    assert.equals((0, newstate_1.valueify)(merged), { a: "one" });
});
wtf.test(`Merge should merge { a: "one" } with { a: null }.`, (assert) => {
    let one = (0, newstate_1.stateify)({ a: "one" });
    let two = (0, newstate_1.stateify)({ a: null });
    let merged = (0, newstate_1.merge)(one, two);
    assert.equals((0, newstate_1.valueify)(merged), { a: null });
});
wtf.test(`Merge should merge { a: "one" } with { a: undefined }.`, (assert) => {
    let one = (0, newstate_1.stateify)({ a: "one" });
    let two = (0, newstate_1.stateify)({ a: undefined });
    let merged = (0, newstate_1.merge)(one, two);
    assert.equals((0, newstate_1.valueify)(merged), { a: "one" });
});
wtf.test(`Merge should merge {} with { a: "one" }.`, (assert) => {
    let one = (0, newstate_1.stateify)({});
    let two = (0, newstate_1.stateify)({ a: "one" });
    let merged = (0, newstate_1.merge)(one, two);
    assert.equals((0, newstate_1.valueify)(merged), { a: "one" });
});
wtf.test(`Merge should merge {} with {}.`, (assert) => {
    ...
    let two = (0, newstate_1.stateify)({});
    let merged = (0, newstate_1.merge)(one, two);
    assert.equals((0, newstate_1.valueify)(merged), {});
});
wtf.test(`Merge should merge {} with {}.`, (assert) => {
    let one = (0, newstate_1.stateify)({});
    let two = (0, newstate_1.stateify)({ a: null });
...
wtf.test(`Merge should merge {} with {}.`, (assert) => {
    ...
    let two = (0, newstate_1.stateify)({ a: undefined });
    let merged = (0, newstate_1.merge)(one, two);
    assert.equals((0, newstate_1.valueify)(merged), { a: undefined });
});
wtf.test(`Merge should merge { a: null } with { a: "one" }.`, (assert) => {
    let one = (0, newstate_1.stateify)({ a: null });
    let two = (0, newstate_1.stateify)({ a: "one" });
    let merged = (0, newstate_1.merge)(one, two);
    assert.equals((0, newstate_1.valueify)(merged), { a: "one" });
});
...
wtf.test(`Merge should merge { a: undefined } with { a: "one" }.`, (assert) => {
    let one = (0, newstate_1.stateify)({ a: undefined });
    let two = (0, newstate_1.stateify)({ a: "one" });
    let merged = (0, newstate_1.merge)(one, two);
    assert.equals((0, newstate_1.valueify)(merged), { a: "one" });
});
...
wtf.test(`Merge should return the correct type for { a: string } and { a: string }.`, (assert) => {
    let one = (0, newstate_1.stateify)({ a: "one" });
    let two = (0, newstate_1.stateify)({ a: "two" });
    let merged = (0, newstate_1.merge)(one, two);
});
wtf.test(`Merge should return the correct type for { a?: string } and { a: string }.`, (assert) => {
    let one = (0, newstate_1.stateify)({});
...
wtf.test(`Merge should return the correct type for { a: string } and { a?: string }.`, (assert) => {
    let one = (0, newstate_1.stateify)({ a: "two" });
    let two = (0, newstate_1.stateify)({});
    let merged = (0, newstate_1.merge)(one, two);
});
wtf.test(`Merge should return the correct type for { a?: string } and { a?: string }.`, (assert) => {
...
wtf.test(`Merge should return the correct type for { a: string } and { a: number }.`, (assert) => {
    let one = (0, newstate_1.stateify)({ a: "one" });
    let two = (0, newstate_1.stateify)({ a: 1 });
    let merged = (0, newstate_1.merge)(one, two);
});
wtf.test(`Merge should return the correct type for { a?: string } and { a: number }.`, (assert) => {
    let one = (0, newstate_1.stateify)({});
...
wtf.test(`Merge should return the correct type for { a: number } and { a: string }.`, (assert) => {
    let one = (0, newstate_1.stateify)({ a: 1 });
    let two = (0, newstate_1.stateify)({ a: "two" });
    let merged = (0, newstate_1.merge)(one, two);
});
wtf.test(`Merge should return the correct type for { a?: number } and { a: string }.`, (assert) => {
...
wtf.test(`Merged objects created from {} and {} should update properly when the first object is updated.`, (assert) => {
    ...
    assert.equals((0, newstate_1.valueify)(merged), {});
    one.update({ key: "one" });
    assert.equals((0, newstate_1.valueify)(merged), { key: "one" });
    one.update({ key: "ONE" });
    assert.equals((0, newstate_1.valueify)(merged), { key: "ONE" });
    one.update({});
    assert.equals((0, newstate_1.valueify)(merged), {});
...
wtf.test(`Merged objects created from {} and {} should update properly when the second object is updated.`, (assert) => {
    ...
    assert.equals((0, newstate_1.valueify)(merged), {});
    two.update({ key: "two" });
    assert.equals((0, newstate_1.valueify)(merged), { key: "two" });
    two.update({ key: "TWO" });
    assert.equals((0, newstate_1.valueify)(merged), { key: "TWO" });
    two.update({});
    assert.equals((0, newstate_1.valueify)(merged), {});
...
wtf.test(`Merged objects created from { key: "one" } and {} should update properly when the first object is updated.`, (assert) => {
    let one = (0, newstate_1.stateify)({ key: "one" });
    let two = (0, newstate_1.stateify)({});
    let merged = (0, newstate_1.merge)(one, two);
    assert.equals((0, newstate_1.valueify)(merged), { key: "one" });
    one.update({ key: "ONE" });
...
wtf.test(`Merged objects created from {} and { key: "two" } should update properly when the first object is updated.`, (assert) => {
    let one = (0, newstate_1.stateify)({});
    let two = (0, newstate_1.stateify)({ key: "two" });
    let merged = (0, newstate_1.merge)(one, two);
    assert.equals((0, newstate_1.valueify)(merged), { key: "two" });
    one.update({ key: "one" });
    assert.equals((0, newstate_1.valueify)(merged), { key: "two" });
...
wtf.test(`Classes should be stateified properly.`, (assert) => {
    let state = (0, newstate_1.stateify)(undefined);
});
wtf.test(`Classes should be valueified properly.`, (assert) => {
    let value = (0, newstate_1.valueify)(undefined);
});
wtf.test(`Objects with readonly members should be assigned and cast properly.`, (assert) => {
    ...
    let state = (0, newstate_1.stateify)(value);
...
wtf.test(`Flatten should flatten three levels of arrays.`, (asserts) => {
    let array_00 = (0, newstate_1.stateify)(["a", "b"]);
    let array_01 = (0, newstate_1.stateify)(["c", "d"]);
    let array_10 = (0, newstate_1.stateify)(["e", "f"]);
    let array_11 = (0, newstate_1.stateify)(["g", "h"]);
    let array_0 = (0, newstate_1.stateify)([array_00, array_01]);
    let array_1 = (0, newstate_1.stateify)([array_10, array_11]);
    let array = (0, newstate_1.stateify)([array_0, array_1]);
    let flattened = (0, newstate_1.flatten)(array);
...
wtf.test(`Flattened arrays should contain items inserted into the original arrays.`, (asserts) => {
    let array_00 = (0, newstate_1.stateify)(["b"]);
    let array_01 = (0, newstate_1.stateify)(["e"]);
    let array_10 = (0, newstate_1.stateify)(["h"]);
    let array_11 = (0, newstate_1.stateify)(["k"]);
    let array_0 = (0, newstate_1.stateify)([array_00, array_01]);
    let array_1 = (0, newstate_1.stateify)([array_10, array_11]);
    let array = (0, newstate_1.stateify)([array_0, array_1]);
    let flattened = (0, newstate_1.flatten)(array);
...
wtf.test(`Flattened arrays should not contain items removed from the original arrays.`, (asserts) => {
    let array_00 = (0, newstate_1.stateify)(["a", "b", "c"]);
    let array_01 = (0, newstate_1.stateify)(["d", "e", "f"]);
    let array_10 = (0, newstate_1.stateify)(["g", "h", "i"]);
    let array_11 = (0, newstate_1.stateify)(["j", "k", "l"]);
    let array_0 = (0, newstate_1.stateify)([array_00, array_01]);
    let array_1 = (0, newstate_1.stateify)([array_10, array_11]);
    let array = (0, newstate_1.stateify)([array_0, array_1]);
    let flattened = (0, newstate_1.flatten)(array);
...
wtf.test(`Flattened arrays should contain arrays inserted into the original arrays.`, (asserts) => {
    let array_00 = (0, newstate_1.stateify)(["a"]);
    let array_01 = (0, newstate_1.stateify)(["b"]);
    let array_02 = (0, newstate_1.stateify)(["c"]);
    let array_10 = (0, newstate_1.stateify)(["d"]);
    let array_11 = (0, newstate_1.stateify)(["e"]);
    let array_12 = (0, newstate_1.stateify)(["f"]);
    let array_20 = (0, newstate_1.stateify)(["g"]);
    let array_21 = (0, newstate_1.stateify)(["h"]);
    let array_22 = (0, newstate_1.stateify)(["i"]);
    let array_0 = (0, newstate_1.stateify)([array_00, array_02]);
    let array_1 = (0, newstate_1.stateify)([array_10, array_12]);
    let array_2 = (0, newstate_1.stateify)([array_20, array_22]);
    let array = (0, newstate_1.stateify)([array_0, array_2]);
    let flattened = (0, newstate_1.flatten)(array);
...
wtf.test(`Flattened arrays should not contain arrays removed from the original arrays.`, (asserts) => {
    ...
    let array_21 = (0, newstate_1.stateify)(["h"]);
    let array_22 = (0, newstate_1.stateify)(["i"]);
    let array_0 = (0, newstate_1.stateify)([array_00, array_01, array_02]);
    let array_1 = (0, newstate_1.stateify)([array_10, array_11, array_12]);
    let array_2 = (0, newstate_1.stateify)([array_20, array_21, array_22]);
    let array = (0, newstate_1.stateify)([array_0, array_1, array_2]);
    let flattened = (0, newstate_1.flatten)(array);
    array_0.remove(1);
    array_1.remove(1);
    array_2.remove(1);
...
wtf.test(`Dynamic members should be updated when the dynamic key is updated.`, (asserts) => {
    let state = (0, newstate_1.make_state)({ one: "one", two: "two" });
    let key = (0, newstate_1.make_state)("one");
...
wtf.test(`Primitive shadow states should synchronize with their source states.`, (assert) => {
    let source = (0, newstate_1.make_state)(false);
...
wtf.test(`Reference shadow states should synchronize with their source states.`, (assert) => {
    let one = {};
    let two = {};
    let source = (0, newstate_1.make_state)(one);
    let shadow = source.shadow();
    shadow.update(two);
...
wtf.test(`Array shadow states should synchronize with their source states using the correct events.`, (assert) => {
    let events = [];
    let source = (0, newstate_1.make_state)([]);
...
wtf.test(`Array states should update properly when observers are attached as a result of the first() element changing.`, (assert) => {
    let state = (0, newstate_1.make_state)(["one"]);
